...
const EditorPage = () => {
  ...
  const fileExist = async (room) => {
    try {
      const response = await axios.post("http://localhost:5000/fileExist", {
      ...
      if (response.data.exists) {
        ...
        return response.data.content; // Return the content of the room
      } else {
      ...
      throw new Error("Could not check file existence. Please try again.");
    }
  };
  ...
  async function runcode() {
    ...
    try {
      ...
      console.log(response.data);
      if (response.data == "") {
      } else toast.success("Running the code");
      editorRef3.current?.setValue(response.data);
    } catch (error) {
      console.log(error);
      if (error.response.status == 400) {
        toast.error("One of Field is empty");
      } else toast.error("Failed to run code");
    }
  }
      ...
      <div style={{ height: "10vh", width: "100vw" }}>
        ...
        >
          ...
          <NavbarContent justify="center">
            <NavbarItem>
              <FaPencilAlt
                onClick={() => reactNavigator(`/whiteboard/${roomId}`)}
              />
            </NavbarItem>
            <NavbarItem>
              <Button color="success" variant="flat" onClick={runcode}>
                Run Code
              </Button>
            </NavbarItem>
            <NavbarItem>
              ...
              <Modal
                ...
                styles={{ backgroundColor: "black", fontFamily: "Manrope" }}
              >
                <ModalContent>
                  {(onClose) => (
                    <>
                      ...
                      <ModalBody>
                        ...
                        >
                          ...
                          <div
                            style={{ display: "flex", fontFamily: "Manrope" }}
                          >
                            <div
                              style={{ padding: "2vw", fontFamily: "Manrope" }}
                            >
                              <Button
                                color="primary"
                                variant="flat"
            ...
            <NavbarItem>
              <Button color="success" variant="flat" onClick={copyRoomId}>
                Copy Room ID
              </Button>
            </NavbarItem>
          </NavbarContent>
          <NavbarContent justify="end">
            <NavbarItem>
              <Button color="danger" className="m-4" onClick={leaveRoom}>
                Leave
              </Button>
            </NavbarItem>
          </NavbarContent>
